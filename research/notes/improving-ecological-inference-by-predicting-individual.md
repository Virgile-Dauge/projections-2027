---
title: Improving Ecological Inference by Predicting Individual
id: improving-ecological-inference-by-predicting-individual
tags:
...
created: '2026-07-21T19:36:06.218306Z'
updated: '2026-07-21T19:42:36.640350Z'
source: https://imai.fas.harvard.edu/research/files/race.pdf
source_domain: imai.fas.harvard.edu
fetched_at: '2026-07-21T19:36:06.218100Z'
...
content_type: paper
deprecated: false
summary: 'Imai & Khanna (2016, Political Analysis 24:263-272, doi:10.1093/pan/mpw001).
  Proposes BISG (Bayesian Improved Surname Geocoding, the ''wru'' R package): predicting
  INDIVIDUAL-level ethnicity from Census surname lists + geocoded voter-registration
  records via Bayes'' rule, then aggregating these individual predictions to get turnout/vote-choice-by-race
  estimates -- bypassing classical ecological inference''s aggregation step. Validated
  against ~9 million Florida voter registration records with self-reported ethnicity
  as ground truth: false-positive rate reduced to 6% (Black) and 3% (Latino) voters
  while true-positive rate held above 80%. Turnout-by-race estimates from BISG show
  substantially less bias and lower RMSE than standard ecological-inference (King
  1997-style) estimates. Methodologically adjacent but distinct from the EI-vs-MRP
  question: this is an individual-level classification approach with real (not simulated)
  ground truth, at a scale (millions of individual voters) that implicitly spans precinct-level
  heterogeneity, but the paper''s accuracy metrics (RMSE, bias) are not reported disaggregated
  at the precinct/polling-station level nor as rank correlations.'
raw_file: raw/improving-ecological-inference-by-predicting-individual.pdf
---
...
2005), mobilization eﬀorts (e.g., Barreto 2007), and the eﬀects of co-ethnic candidates and repre-
...
However, such eﬀorts face a well-known methodological obstacle, known as the ecological in-
...
263
 at Princeton University on April 22, 2016
...
additional information such as party registration, this methodological framework oﬀers improve-
...
Pr ðRi ¼ rjSi ¼ sÞ, the racial composition of each geolocation (e.g., Census blocks and voting pre-
...
level covariates available in the voter ﬁles. In this article, we focus on age, gender, and party
...
sidering two possibilities. The ﬁrst approach requires that researchers have information about the
...
Pr ðXi ¼ xjGi ¼ g; Ri ¼ rÞ ¼ Pr ðXi ¼ x; Ri ¼ rjGi ¼ gÞ=P
x02X Pr ðXi ¼ x0; Ri ¼ rjGi ¼ gÞ. The ﬁrst
...
ual records. Our data are based on statewide voter ﬁles and come from L2 (formerly Labels & Lists,
...
ical units, including blocks, tracts, and precincts. The summary ﬁle contains raw counts
...
each voter, we ﬁnd the race with the greatest predicted probability and classify the voter as be-
...
the goal is to minimize both types of error, there is a clear trade-oﬀbetween the two.
...
Table 1
Empirical validation of individual-level race classification using the Florida registration records
Name
Name
Name
Name
...
A more comprehensive comparison of predictions, while recognizing the trade-oﬀbetween false
...
ciﬁcity) for a variety of classiﬁcation thresholds. Since it is desirable to have a higher true positive
...
method correctly classiﬁes over 80% of Blacks and Latinos, while only misclassifying 6% of non-
...
Name Only (.93)
Fig. 1
ROC curves for the proposed race prediction methods. ROC curves plot true positive rate (vertical
...
turnout variable for voter i. For the purpose of comparison, we also compute the prediction based on
the Census surname alone and compute Pn
i¼1 Pr ðRi ¼ rjSiÞYi=Pn
i¼1 Pr ðRi ¼ rjSiÞ.
...
dictions and signiﬁcantly improves the estimation of aggregate-level turnout for each racial group
...
Fieldhouse, E., and D. Cutts. 2008. Diversity, density and turnout: The effect of neighbourhood ethno-religious com-
position on voter turnout in Britain. Political Geography 27(5):530–48.
