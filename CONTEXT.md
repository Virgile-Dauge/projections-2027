...
- **Tranche départementale** — référentiel d'affichage de la réserve sur la carte : le quantile est calculé parmi les unités du même département (bureaux entre eux, communes entre elles), jamais sur la France entière. Échelle d'action locale (« où aller autour de moi »), pas un palmarès national : en échelle nationale, les zones où un bloc est structurellement faible sortent uniformément pâles et la carte décourage au lieu d'orienter. Garde-fous : le référentiel est toujours annoncé avec la tranche, et le niveau absolu (inscrits mobilisables) reste affiché. Le rapport de force, couche de contexte, reste en quantile national par bloc. Arbitrage complet (palmarès national vs actionnabilité locale, garde-fous, périmètre) : `docs/adr/0005-echelle-departementale-reserve.md`.
